...
!!$   selected real space wavefunction in a3 direction to standard
!output
...
!!$                                no. plane waves, and adjusted 'c'
!value
!!$
!!$   options are -f filename -s spin -k k-point -b band -x coord -y
!coord
...
!!$ This script has been modified by Tess Smidt to print
!!$ wavefunction over x,y,z grid specified by nb#max
!!$ in each direction.
...
dimension wk(3),xyz(3),wkpg(3),ig(3),rxyz(3)
complex*16 csum
...
!!$ parse arguments
call parse(filename,spin,kpoint,band)
...
do ix=0,2*nb1max
   x=dble(ix)/dble(1+2*nb1max)
   xyz(1)=x
   do iy=0,2*nb2max
      y=dble(iy)/dble(1+2*nb2max)
      xyz(2)=y
      do iz=0,2*nb3max
...
!!$ output z*a3mag for units of Angstroms
         rxyz(1)=a1(1)*xyz(1)+a2(1)*xyz(2)+a3(1)*xyz(3)
         rxyz(2)=a1(2)*xyz(1)+a2(2)*xyz(2)+a3(2)*xyz(3)
         rxyz(3)=a1(3)*xyz(1)+a2(3)*xyz(2)+a3(3)*xyz(3) 
         write(6,*) sngl(rxyz(1)),sngl(rxyz(2)),sngl(rxyz(3)),sngl(real(csum)),sngl(dimag(csum))
      enddo
   enddo
enddo
...
subroutine parse(filename,spin,kpoint,band)
character*75 filename
...
subroutine help
  write(6,*) "syntax: WaveTransPlotMod -f file -s spin -k k-point -b band &
       "
