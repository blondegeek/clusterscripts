...
# magx, magy, and magz attributes added (default only provides magx)
# Tess Smidt
# file is should be copied to
# pymatgen/2.7.9/lib/python2.7/site-packages/pymatgen/io/vaspio
...
class Outcar(object):
    ...
    def __init__(self, filename):
        ...
        with zopen(filename, "r") as f:
			...
			data.reverse()
			magy = [dict(zip(header,v)) for v in data]
			read_charge_mag = False
		    elif clean == "magnetization (z)":
			data.reverse()
			magz = [dict(zip(header,v)) for v in data]
			read_charge_mag = False
		    else:
	    ...
	    self.magnetizationx = tuple(magx)
